...
p.add_argument("--colored", action="store_true", help="assign flat colours (for VACE keep-unchanged colour carry-over)")
...
for i, spec in enumerate([s for s in args.figures.split(",") if s]):
    # name:action[@f0-f1]  e.g. mom:point@12-40 (action only inside that frame window, idle elsewhere)
    name, _, rest = spec.partition(":")
    action, _, win = rest.partition("@")
    ...
    root = ms.add_mannequin(name, loc, height=h, facing_deg=args.facing, action=action, frames=args.frames, window=window)
